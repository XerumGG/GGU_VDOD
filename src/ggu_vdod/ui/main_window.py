...
class GGUVDODApp(tk.Tk):
    def __init__(self):
        ...
        # Keep startup typography predictable. Zoom is intentionally session-only
        # so an accidental Ctrl+wheel cannot leave the next launch oversized.
        self.zoom_percent_var = tk.IntVar(value=ZOOM_DEFAULT_PERCENT)
        ...
        active_bindings = [binding for binding in self.key_bindings.values() if binding != "None"]
        if len(active_bindings) != len(set(active_bindings)):
            # A duplicated shortcut makes one action unreachable. Restore a known
            # working set rather than carrying a broken configuration across runs.
            self.key_bindings = dict(DEFAULT_KEY_BINDINGS)
    ...
    def _build_ui(self):
        ...
        title_label = self._label(content, text=APP_NAME, font=("Segoe UI", 24, "bold"))
        ...
        self.url_text = self._scrolled_text(content, BG_ENTRY, FG, height=5, wrap="word", font=("Segoe UI", 11))
    ...
    def _build_menu_bar(self):
        ...
        view_menu.add_command(label="Zoom in", accelerator="Ctrl + +", command=lambda: self._change_zoom(ZOOM_STEP_PERCENT))
        view_menu.add_command(label="Zoom out", accelerator="Ctrl + -", command=lambda: self._change_zoom(-ZOOM_STEP_PERCENT))
        view_menu.add_command(label="Reset zoom", accelerator="Ctrl + 0", command=lambda: self._apply_zoom(ZOOM_DEFAULT_PERCENT))
    ...
    def _schedule_scrollregion_update(self, _event=None):
    ...
    def _cancel_smooth_scroll(self):
        self._scroll_target = None
    ...
    def _show_key_bindings_preferences(self):
        ...
        def apply_preferences():
            ...
            active_bindings = [binding for binding in selected_bindings.values() if binding != "None"]
            if len(active_bindings) != len(set(active_bindings)):
                messagebox.showwarning(
                    "Duplicate shortcut",
                    "Each zoom action needs a different shortcut. Choose None for an unused action.",
                    parent=dialog,
                )
                return
            self.scroll_speed_var.set(clamp_scroll_speed(dialog_speed.get()))
            self.key_bindings = selected_bindings
            self._bind_configured_shortcuts()
            save_config(self._settings_from_ui())
            dialog.destroy()
        ...
        zoom_controls = self._frame(body)
        zoom_controls.pack(fill="x", pady=(10, 0))
        self._label(zoom_controls, text="Zoom controls:", width=14, anchor="w").pack(side="left")
        self._button(zoom_controls, "−", lambda: self._change_zoom(-ZOOM_STEP_PERCENT)).pack(side="left")
        self._button(zoom_controls, "Reset", lambda: self._apply_zoom(ZOOM_DEFAULT_PERCENT)).pack(side="left", padx=6)
        self._button(zoom_controls, "+", lambda: self._change_zoom(ZOOM_STEP_PERCENT)).pack(side="left")

        self._label(
            body,
            text=f"Zoom range: {ZOOM_MIN_PERCENT}% to {ZOOM_MAX_PERCENT}%. Hold Ctrl and use the mouse wheel to zoom. View also has zoom controls.",
